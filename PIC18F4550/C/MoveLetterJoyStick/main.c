...
void main() {
    ...
    while(1){
        ...
        if ( x > 750 && position_x < 83 ) {
            int delta = map(x, 750, 1023, 1, 5);
            position_x += delta;
        }

        if ( x < 250 && position_x > 0 ) {
            int delta = map(x, 0, 249, 5, 1);
            position_x -= delta;
        }

        if ( y > 750 && position_y < 6 ) {
            position_y += 1;
        }

        if ( y < 250 && position_y > 0 ) {
            position_y -= 1;
        }

        Nokia_Clear(); // Clear Nokia display
        Nokia_PositionXY(position_x, position_y);  // Set X and Y position for printing
        Nokia_SendString("X");
    };
}
