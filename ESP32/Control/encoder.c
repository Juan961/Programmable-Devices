...
volatile unsigned long last_rising = 0;  // last timestamp in microseconds
...
void calculateVelocity() {
  // Calculate using the latest time_elapsed (in microseconds)
  float time_elapsed_s = time_elapsed / 1000000.0;  // convert microseconds to seconds

  float velocity = distance_between_holes / time_elapsed_s;
  float angular_velocity_rs = velocity / encoder_radius;

  angular_velocity = angular_velocity_rs * (60 / (2 * PI));
}
